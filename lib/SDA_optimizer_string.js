...
var SDA = function(system_settings){
  ...
  // If error check is true, flag system design failure, and report notice to user.
  if(error_check.array_test_2){ report_error( 'Maximum system voltage exceeds the maximum voltage allowed by code.' );}
  error_check.power_check_array = array.pmp > 11500;
  // If error check is true, flag system design failure, and report notice to user.
  if( error_check.power_check_array ){ report_error( 'Array total power exceeds 10kW' );}
  error_check.oversize_check_inverter = array.pmp > ( inverter.max_ac_output_current * interconnection.grid_voltage * inverter.oversize_limit );
  // If error check is true, flag system design failure, and report notice to user.
  if( error_check.oversize_check_inverter ){ report_error( 'PV output power exceeds the inverter maximum.' );}
  error_check.optimizer_micro_branch_too_many_modules = array.largest_string > optimizer.max_optis_per_string;
  // If error check is true, flag system design failure, and report notice to user.
  if(error_check.optimizer_micro_branch_too_many_modules ){ report_error( 'The system has too many modules per array source circuit.' );}
  error_check.optimizer_micro_branch_too_few_modules = array.smallest_string < optimizer.min_optis_per_string;
  // If error check is true, flag system design failure, and report notice to user.
  if(error_check.optimizer_micro_branch_too_few_modules ){ report_error( 'The system has too few modules per array source circuit.' );}
  error_check.optimizer_micro_branch_too_much_power = source.max_power > optimizer.max_power_per_string;
  // If error check is true, flag system design failure, and report notice to user.
  if(error_check.optimizer_micro_branch_too_much_power ){ report_error( 'The string source power has exceeded the manufacturer\'s limit.' );}
  ...
  error_check.module_isc_too_high = module.isc > optimizer.opti_max_isc ;
  // If error check is true, flag system design failure, and report notice to user.
  if(error_check.module_isc_too_high ){ report_error( 'Module exceeds optimizer\'s short circuit current maximum.' );}
  ...
  circuit_names.forEach(function(circuit_name, i){
    ...
    circuit.location = sf.index( ['Free air', 'Conduit/Ext.', 'Conduit/Int.'], circuit.id );
    circuit.material = 'CU';
    circuit.type = sf.index( ['PV Wire, bare solid', 'THWN-2', 'THWN-2'], circuit.id );
    ...
    if(circuit.location==='Free air'){
      table = tables[10];
    } else {
      table = tables[4];
    }
    // Use Table 4 or 10, lookup: circuit.min_req_cond_current, find the next highest value, return the index column.
    circuit.conductor_current = sf.lookup( circuit.min_req_cond_current, table, 0, true);
    // Use Table 4 or 10, lookup: circuit.conductor_current, return the first column.
    circuit.conductor_size_min = sf.lookup( circuit.conductor_current, table );
    if( circuit_name === 'exposed source circuit wiring' ){
      if( Number(circuit.conductor_size_min) > 10 ){
    ...
    circuit.conductor_size_OCPD_min = circuit.conductor_size_min;
    if( circuit_name === 'inverter ac output circuit' ){
      if( circuit.OCPD === 15){
        circuit.conductor_size_OCPD_min = '14';
      } else if( circuit.OCPD === 20){
        circuit.conductor_size_OCPD_min = '12';
      } else if( circuit.OCPD === 25){
        circuit.conductor_size_OCPD_min = '10';
      } else if( circuit.OCPD === 30){
        circuit.conductor_size_OCPD_min = '10';
      } else if( circuit.OCPD === 35){
        circuit.conductor_size_OCPD_min = '8';
      } else if( circuit.OCPD === 40){
        circuit.conductor_size_OCPD_min = '8';
      }
      circuit.conductor_size_min = Number(circuit.conductor_size_OCPD_min) < Number(circuit.conductor_size_min) ? circuit.conductor_size_OCPD_min : circuit.conductor_size_min;
    }
    // Use Table 5 or 11, lookup: circuit.conductor_size_min, return the first column.
    if(circuit.location==='Free air'){
      table = tables[11];
    } else {
      table = tables[5];
    }
    circuit.conductor_current = sf.lookup( circuit.conductor_size_min, table, 1);
    ...
    if( circuit_name === 'exposed source circuit wiring' ){
      circuit.conductor_size_min = circuit.conductor_size_min + ', ' + circuit.conductor_size_min + ', ' + '6';
    } else {
  ...
  interconnection.check_a = ( ( interconnection.inverter_output_cur_sum * 1.25 ) + interconnection.supply_ocpd_rating ) > interconnection.bussbar_rating;
  interconnection.check_b = ( interconnection.inverter_output_cur_sum * 1.25 ) + interconnection.supply_ocpd_rating > interconnection.bussbar_rating * 1.2;
  interconnection.check_c = ( interconnection.inverter_ocpd_dev_sum + interconnection.load_breaker_total ) > interconnection.bussbar_rating;
  error_check.interconnection_bus_pass = sf.and( interconnection.check_a, interconnection.check_b, interconnection.check_c );
  interconnection.interconnection_bus_pass = error_check.interconnection_bus_pass;
  // If error check is true, flag system design failure, and report notice to user.
