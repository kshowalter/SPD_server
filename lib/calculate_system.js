...
var lookup = function(search_key, table, col, reverse){
  ...
  if( isNaN(search_key) ){ // search_key is string, not a number
    ...
    for( var key in table){
      ...
      //if( key_number === search_value && !reverse ){ // Exact match
...
var calculate_system = function(system_settings){
  ...
  ******************/
  ...
  circuits['INTERMODULE WIRING'].total_CC_conductors = ( array.num_of_strings * 2 );
  ...
  circuits['PV DC COMBINED CIRCUITS'].total_CC_conductors = ( inverter.mppt_channels * 2 );
  ...
  circuit_names.forEach(function(circuit_name, i){
    ...
    console.log(circuit.power_type === 'DC', circuit.min_req_OCPD_current_DC, circuit.min_req_cond_current_1 );
