...
var lookup = function(search_key, table, col, reverse){
  ...
  if( isNaN(search_key) ){ // search_key is string, not a number
    ...
    for( var key in table){
      ...
      if( key_number === search_value || next ){ // Exact match
        key_last_match = key;
        break;
...
var calculate_system = function(system_settings){
  ...
  //array.num_of_strings
  //array.smallest_string
  //array.largest_string
  //array.num_of_modules
  ...
  if( array.num_of_modules > max_modules ){ notes.warnings.push('Module total exceeds the number possible with selected string sizes.'); }
  if( array.num_of_modules < min_modules ){ notes.warnings.push('Module total is less than the number possible with selected string sizes.'); }

  if( array.num_of_strings && ( array.smallest_string !== array.largest_string ) ){
  ...
  array.circuits_per_MPPT = array.num_of_strings / inverter.mppt_channels;
  ...
  ******************/
  ...
  circuits['INTERMODULE WIRING'].max_current = source.isc;
  circuits['INTERMODULE WIRING'].max_voltage = source.voc;
  circuits['INTERMODULE WIRING'].total_conductors = ( array.num_of_strings * 2 ) +1;
  circuits['INTERMODULE WIRING'].temp_adder = lookup( module.array_offset_from_roof, tables[1] );
  ...
  circuits['PV DC COMBINED CIRCUITS'].max_current = source.isc * array.circuits_per_MPPT;
  circuits['PV DC COMBINED CIRCUITS'].max_voltage = source.voc;
  circuits['PV DC COMBINED CIRCUITS'].total_conductors = ( inverter.mppt_channels * 2 ) +1;
  ...
  circuit_names.forEach(function(circuit_name, i){
    ...
    circuit.conductors_adj_factor = lookup( circuit.total_conductors , tables[3] );
    ...
    circuit.min_req_OCPD_current = sf.max( circuit.min_req_cond_current_1, circuit.min_req_cond_current_2, circuit.min_req_cond_current_3  );
    circuit.OCPD = lookup( circuit.min_req_OCPD_current, tables[8], 0, true);
    circuit.conductor_current = lookup( circuit.OCPD, tables[4], 0, true );
    ...
    circuit.OCPD_required = sf.index( [false, array.num_of_strings > 2, false, true, true ], circuit.id );
    circuit.ocpd_type = sf.index( ['NA', 'PV Fuse', 'NA', ' Circuit Breaker', 'Circuit Breaker'], circuit.id );

    //circuit.OCPD = parseFloat(circuit.OCPD).toFixed(2);
