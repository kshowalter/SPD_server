...
var lookup = function(search_key, table_name){
  ...
  if( isNaN(search_key) ){ // search_key is string, not a number
    ...
    var pir_key = Object.keys(table)[0];
    console.log('init pir', pir_key);

    for( var key in table){
      console.log('/ ', key, table[key] );
      key = Number(key);
      if( key === search_value ){ // Exact match
        console.log('exact');
        return table[key];
      } else if( key < search_value ){ // Possible match, but we might find a closer one.
        console.log('possible');
        pir_key = key;
        console.log('pir', pir_key);
      } else { // key > search_value , this fails, return last acceptable value
        console.log('over');
        return table[pir_key];
      }
    }
  }
...
console.log( 'TABLE TEST', lookup('9.0',1) );
...
var calculate_system = function(system_settings){
  ...
  ******************/
  ...
  if( interconnection.bussbar_rating >=
      ( ( inverter.max_ac_ouput_current * 1.25 ) +
  ...
  if ( ( inverter.max_ac_ouput_current * 1.25 +
               interconnection.supply_ocpd_rating ) <=
