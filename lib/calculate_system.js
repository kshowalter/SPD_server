...
var calculate_system = function(system_settings){
  ...
  array.current_check_inverter = array.isc_adjusted > inverter.imax_channel;
  ...
  if( array.current_check_inverter ){ notes.warnings.push('PV source circuit maximum current exceeds the inverter maximum DC operating current per input.'); }
  ...
  ******************/
  ...
  circuit_names.forEach(function(circuit_name, i){
    ...
    //if( circuit.ocpd_check_2 ){ notes.errors.push( circuit_name + ': Adj. Conductor Amapacity > OCPD Rating '); }
    circuit.OCPD_min = parseFloat(circuit.OCPD_min).toFixed(2);
