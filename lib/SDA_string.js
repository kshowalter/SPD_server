...
var SDA = function(system_settings){
  ...
  array.min_voltage = array.smallest_string * module.vmp * ( 1 + module.tc_voc_percent / 100 * ( array.max_temp - 25 ) );
  ...
  error_check.array_mppt_test_1 = array.num_of_strings > inverter.mppt_channels;
  // If error check is true, flag system design failure, and report notice to user.
  if(error_check.array_mppt_test_1 ){ report_error( 'The number of strings exceeds the number of inverter MPPT channels' );}
  ...
  error_check.oversize_check_inverter = array.pmp > ( inverter.nominal_ac_output_power * inverter.oversize_limit );
  // If error check is true, flag system design failure, and report notice to user.
  ...
  circuits['exposed source circuit wiring'].max_current_adjusted = array.combined_isc_adjusted;
  ...
  circuits['pv dc source circuits'].max_current_adjusted= array.combined_isc_adjusted;
  ...
  circuits['inverter ac output circuit'].max_current_adjusted = inverter.max_ac_output_current;
