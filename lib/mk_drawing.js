...
var mk_drawing = function(settings){
  ...
  settings.drawing_settings.size.DC_wire_offset.max = settings.drawing_settings.size.DC_wire_offset.min +
                                                   settings.state.system.array.num_of_strings *
                                                   settings.drawing_settings.size.DC_wire_offset.base;
  settings.drawing_settings.size.DC_wire_offset.ground = settings.drawing_settings.size.DC_wire_offset.max +
                                                      settings.drawing_settings.size.DC_wire_offset.base*1;
  ...
  var max_displayed_modules = 15;
  settings.drawing.break_string = [];
  settings.drawing.module_count = [];
  ...
  if( settings.state.system.array.num_of_strings === 1 ){
    settings.drawing.module_count[0] = settings.state.system.array.largest_string;
  } else if( settings.state.system.array.num_of_strings === 2 ){
    settings.drawing.module_count[0] = settings.state.system.array.smallest_string;
    settings.drawing.module_count[1] = settings.state.system.array.largest_string;
    ...
    settings.drawing.module_count[0] = settings.state.system.array.smallest_string;
    settings.drawing.module_count[1] = middle_string_number;
    settings.drawing.module_count[2] = settings.state.system.array.largest_string;
  } else if( settings.state.system.array.num_of_strings >= 4){
    for( var i in _.range(settings.state.system.array.num_of_strings)) {
      settings.drawing.module_count[i] = array.largest_string;
    }
    settings.drawing.module_count[0] = array.smallest_string;

    var num_modules_to_remove = max_modules - settings.state.system.array.num_of_modules;
    var i = 1;
    while( num_modules_to_remove > 0 ){
      settings.drawing.module_count[i]--;
      i++;
  ...
  settings.drawing.module_count.forEach(function(displayed_modules, i){
    settings.drawing.displayed_modules[i] = settings.drawing.module_count[i];
    var width = 0;
    if( settings.drawing.displayed_modules[i] > max_displayed_modules ){
      width += settings.drawing_settings.size.string.gap_missing;
      width += settings.drawing_settings.size.module.w;
      settings.drawing.displayed_modules[i] = max_displayed_modules - 1;
      settings.drawing.break_string[i] = true;
    }
    width += settings.drawing_settings.size.module.w * (settings.drawing.displayed_modules[i]);
  ...
  } else if( settings.state.system.config.system_type === 'optimizer' ){
    settings.drawing_settings.sheets.push(
      {
        sheet_id: 'W-001_optimizer_string',
        num: 'W-001',
        desc: 'Wiring Diagram'
      }
    );
