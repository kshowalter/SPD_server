  ...
  for( var section_name in inputs ){
    if( inputs.hasOwnProperty(section_name) ){
      if( inputs[section_name].constructor === Object ) {
        ...
        logger.info('error: section not object');
      }
    }
  }
...
f.table_name = function(name){
  return f.pretty_name(name).toUpperCase();
};
...
f.toggle_module = function(element){
  //logger.info('switch', element, element.classList );
  ...
  //logger.info("settings.user_system.selected_modules:", settings.user_system.selected_modules);
...
logger.info('geocode failed', jqxhr, textStatus, error);
...
f.get_risk_categories = function(data){
  ...
  if( data !== undefined ){
    ...
    logger.info('Wind data from address: ', data);
    ...
    logger.info("state.system.location.wind=>" + JSON.stringify(state.system.location.wind));
    update();
...
//    logger.info('New location from address', data);
...
f.set_new_coordinates = function(system_id, geocode_info){
  logger.info('New location from address', system_id, geocode_info);//, location_data);
  ...
  try{
    ...
    logger.info('wind_data', wind_data);
    if( wind_data && ! _.isEmpty(wind_data) ){
      logger.info('wind_data returned');
      ...
      logger.info('wind_data NOT returned');
    }
    //});
