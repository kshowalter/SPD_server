...
var Drawing = function(settings){
  ...
  drawing.block_start = function(name) {
    if( typeof name === 'undefined' ){ // if name argument is submitted
      logger.info('Error: name required');
    } else {
      var blk;
      block_active = name;
      if( settings.drawing.blocks[block_active] !== undefined ){
        //logger.info('Error: block already exists');
      }
  ...
  if( ! (layer_name in layers) ){ logger.info("error, layer does not exist, using current");}
  ...
  col_size: function(col, size){
    if( typeof col === 'string' ){
      if( col === 'all'){
        ...
        size = Number(size);
        if( isNaN(size) ){
          logger.info('Error: column wrong');
        } else {
