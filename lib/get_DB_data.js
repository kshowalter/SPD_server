...
var get_DB_data = function(system_id, callback){
  var g_connection;
...
var get_DB_system_ids = function(callback){
  var g_connection;
  ...
  var run_function_when_all_queries_are_ready = f.mk_ready_count(3, function(query_responces){
    ...
    if( query_responces ){
      //*
      var systems_type_lists = {};
      for( var n in query_responces){
        var response_data = query_responces[n].data.rows;
        if( response_data[0] ){
          var device_type_id = response_data[0].DEVICE_TYPE_ID;
          systems_type_lists[system_types[device_type_id]] = response_data;
        }
      }
      callback(systems_type_lists);
      //*/
