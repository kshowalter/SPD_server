...
var mk_calculation_tables = require('./mk_calculation_tables.js');
var mk_system_requirements = require('./mk_system_requirements.js');
var mk_db_info_tables = require('./mk_db_info_tables.js');
...
var process_system = function(settings){
  ...
  if( system.inverter.grid_voltage !== 240 ){
    state.notes.errors.push('Three-phase systems are not currently accepted.');
  }
  ...
  if( !system.inverter.nominal_ac_output_power ){
    system.inverter.nominal_ac_output_power = system.inverter.max_ac_output_current * system.inverter.grid_voltage;
  }
  ...
  } else if(system.inverter.device_type_id === 56){
  ...
  settings.state.calculation_tables = mk_calculation_tables(settings);
  settings.state.system_requirements = mk_system_requirements(settings);
  settings.state.db_info_tables = mk_db_info_tables(settings);
