var map_DB_data = function(DB_data){
  ...
  if( data.inverter.device_type_id === 53 ){
    ...
    if( data.inverter.device_model_number &&
        data.inverter.device_model_number.split('-')[0] &&
        data.inverter.device_model_number.split('-')[0].slice(-1) === 'H' ){
      data.inverter.oversize_limit = 1.55;
      data.optimizer['max_power_per_string'] = 5700;
      if( data.inverter.device_model_number === 'SE7600H-US' ||
          data.inverter.device_model_number === 'SE10000H-US'){
        data.optimizer['max_power_per_string'] = 6000;
      }
    } else {
