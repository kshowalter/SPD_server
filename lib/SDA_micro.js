...
var SDA = function(system_settings){
  ...
  circuits['Manufacturer cable'].max_current = source.current;
  circuits['PV Microinverter AC sources'].max_current = source.current;
  circuits['Manufacturer cable'].max_voltage = inverter.grid_voltage;
  ...
  circuits['Manufacturer cable'].total_cc_conductors = '-';
  ...
  circuit_names.forEach(function(circuit_name, i){
    ...
    circuit.location = sf.index( ['Free air', 'Conduit/Int.'], circuit.id );
    circuit.material = 'CU';
    circuit.type = sf.index( ['TC-ER', 'THWN-2'], circuit.id );
    circuit.volt_rating = 600;
    circuit.wet_temp_rating = 90;
    circuit.conduit_type = sf.index( ['-', 'Metallic'], circuit.id );
    circuit.power_type = sf.index( ['AC', 'AC'], circuit.id );
    ...
    circuit.OCPD_required = sf.index( [ false, true ], circuit.id );
    circuit.ocpd_type = sf.index( ['NA', 'Cir. Breaker'], circuit.id );
